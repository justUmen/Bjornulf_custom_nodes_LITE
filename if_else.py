...
class MatchTextToInput30:
    ...
    def INPUT_TYPES(cls):
        inputs = {
            ...
            "optional": {
                **{f"input_{i}": (Everything("*"), {"forceInput": True}) for i in range(1, 31)},
                **{f"text_{i}": ("STRING", {"default": ""}) for i in range(1, 31)},
                "use_regex": ("BOOLEAN", {"default": True}),
            }
        }
        return inputs
    ...
    def match_text(self, input_text, use_regex=True, **kwargs):
        # Collect inputs and texts in lists
        inputs = [kwargs.get(f"input_{i}") for i in range(1, 31)]
        texts = [kwargs.get(f"text_{i}", "") for i in range(1, 31)]
